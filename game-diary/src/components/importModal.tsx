...
import handleCreateDiaryFromTest from 'src/hooks/handleCreateDiaryFromTest';
...
const ImportModal = ({ onClose, isDarkMode }: ExportModalProps) => {
  // const exportText = container
  //   .resolve<IDiaryExporter>('IDiaryExporter')
  //   .exportText();
  const [textData, setTestData] = useState('');
  return (
    <Overlay onClose={onClose} isDarkMode={isDarkMode}>
      ...
      <div className="gap-2 flex justify-start mt-4">
        <button
          ...
          onClick={() => handleCreateDiaryFromTest(textData)}
        >
          インポート
        </button>
        <input
