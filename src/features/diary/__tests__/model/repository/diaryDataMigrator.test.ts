import { MockV0StorageService } from '@features/diary/__tests__/__mocks__/mockV0StorageService';
...
describe('DiaryDataMigrator class tests', () => {
  ...
  describe('v0 storage service mock', () => {
    ...
    beforeEach(() => {
      const json = storage.getItem(STORAGE_CONSTANTS.GAME_DATA_NAME_LIST);
      if (json === null) {
        throw Error;
      }
    ...
    it('should store currentGameDataKey in current_game_data_name', () => {
      expect(storage.getItem(STORAGE_CONSTANTS.CURRENT_GAME_DATA_NAME)).toBe(
        'testKey0'
      );
    });
  ...
  describe('after migration', () => {
    ...
    beforeEach(() => {
      ...
      const s = storage.getItem(STORAGE_CONSTANTS.DIARY_NAME_LIST);
      if (s === null) {
        throw Error;
      }
      diaryNameListJson = s;
    ...
    it('should rename current_game_data_name to current_diary_key', () => {
      expect(
        storage.getItem(STORAGE_CONSTANTS.CURRENT_GAME_DATA_NAME)
      ).toBeNull();
      expect(storage.getItem(STORAGE_CONSTANTS.CURRENT_DIARY_KEY)).toBe(
        'testKey0'
      );
    });
